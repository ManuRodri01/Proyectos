...
import manuRodri.intercambioCartas.Exception.UsuarioYaExisteException;
...
public class AuthService {
    ...
    public AuthResponse login(LoginRequestDTO loginRequest) {
        ...
        UserDetails userDetails = personaRepository.findByNombre(loginRequest.nombreUsuario())
                .orElseThrow(() -> new PersonaNoEncontradaException(loginRequest.nombreUsuario()));

        return new AuthResponse(jwtService.generarToken(userDetails));
    }
    ...
    public AuthResponse register(RegisterRequestDTO registerRequest) {
        ...
        return new AuthResponse(jwtService.generarToken(persona));
    }
}
